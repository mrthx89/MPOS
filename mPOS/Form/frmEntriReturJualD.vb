...
Public Class frmEntriReturJualD
    ...
    Private Sub RefreshDataSO()
        ...
        Try
            SQL = "SELECT HJualD.NoID, HJual.Kode, HJual.Tanggal, HBarang.Kode, HBarang.Nama, (HJualD.Qty*HJualD.Konversi)-ISNULL((SELECT SUM(HReturJualD.Qty*HReturJualD.Konversi) FROM HReturJualD INNER JOIN HReturJual ON HReturJual.NoID=HReturJualD.IDHeader WHERE HReturJualD.IDJualD=HJualD.NoID AND HReturJualD.NoID<>" & NoID & "),0) AS SisaQty " & vbCrLf & _
                              " FROM HJual " & vbCrLf & _
                              " INNER JOIN HJualD ON HJual.NoID=HJualD.IDHeader " & vbCrLf & _
                              " INNER JOIN HBarang ON HBarang.NoID=HJualD.IDBarang " & vbCrLf & _
                              " WHERE (HJualD.Qty*HJualD.Konversi)-ISNULL((SELECT SUM(HReturJualD.Qty*HReturJualD.Konversi) FROM HReturJualD INNER JOIN HReturJual ON HReturJual.NoID=HReturJualD.IDHeader WHERE HReturJualD.IDJualD=HJualD.NoID AND HReturJualD.NoID<>" & NoID & "),0)>=1 " & vbCrLf & _
                              " AND HBarang.NoID=" & NullToLong(txtIDBarang.EditValue) & " AND HJual.IDCustomer=" & NullToLong(IDCustomer)
    ...
    Private Sub frmEntriUser_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            Else
                SQL = "SELECT * FROM HReturJualD WHERE NoID= " & NoID
                EksekusiDataset(ds, "HReturJualD", SQL)
                If ds.Tables("HReturJualD").Rows.Count >= 1 Then
                    txtIDBarang.EditValue = NullToLong(ds.Tables("HReturJualD").Rows(0).Item("IDBarang"))
                    txtSatuan.EditValue = NullToLong(ds.Tables("HReturJualD").Rows(0).Item("IDSatuan"))
                    txtPenjualan.EditValue = NullToLong(ds.Tables("HReturJualD").Rows(0).Item("IDJualD"))
                    txtKonversi.EditValue = NullToDbl(ds.Tables("HReturJualD").Rows(0).Item("Konversi"))
                    txtQty.EditValue = NullToDbl(ds.Tables("HReturJualD").Rows(0).Item("Qty"))
                    txtHarga.EditValue = NullToDbl(ds.Tables("HReturJualD").Rows(0).Item("Harga"))
                    txtDiscProsen1.EditValue = NullToDbl(ds.Tables("HReturJualD").Rows(0).Item("DiscProsen1"))
                    txtDiscProsen2.EditValue = NullToDbl(ds.Tables("HReturJualD").Rows(0).Item("DiscProsen2"))
                    txtDiscRp.EditValue = NullToDbl(ds.Tables("HReturJualD").Rows(0).Item("DiscRp"))
                    txtJumlah.EditValue = NullToDbl(ds.Tables("HReturJualD").Rows(0).Item("Jumlah"))
                    txtKeterangan.Text = NullToStr(ds.Tables("HReturJualD").Rows(0).Item("Keterangan"))
                End If
            End If
            HitungTotal()
    ...
    Private Function IsValidasi() As Boolean
        ...
        If txtHarga.EditValue <= 0 Then
            ...
            End If
        End If
        If txtPenjualan.Text <> "" Then
            SQL = "SELECT (HJualD.Qty*HJualD.Konversi)-ISNULL((SELECT SUM(HReturJualD.Qty*HReturJualD.Konversi) FROM HReturJualD INNER JOIN HReturJual ON HReturJual.NoID=HReturJualD.IDHeader WHERE HReturJualD.IDJualD=HJualD.NoID AND HReturJualD.NoID<>" & NoID & "),0) AS SisaQty " & vbCrLf & _
                  " FROM HJualD " & vbCrLf & _
                  " WHERE HJualD.NoID=" & NullToLong(txtPenjualan.EditValue)
    ...
    Private Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
        Try
            If IsValidasi() Then
                If pStatusBaru Then
                    NoID = GetNewID("HReturJualD")
                    SQL = "INSERT INTO [HReturJualD] ([NoID],[IDHeader],[IDJualD],[IDBarang],[IDSatuan],[Konversi],[Qty],[Harga],[DiscProsen1],[DiscProsen2],[DiscRp],[Jumlah],[Keterangan]) VALUES (" & vbCrLf & _
                          NoID & ", " & IDHeader & ", " & NullToLong(txtPenjualan.EditValue) & ", " & NullToLong(txtIDBarang.EditValue) & ", " & NullToLong(txtSatuan.EditValue) & ", " & FixKoma(txtKonversi.EditValue) & ", " & FixKoma(txtQty.EditValue) & ", " & vbCrLf & _
                ...
                Else
                    SQL = "UPDATE [HReturJualD] SET " & vbCrLf & _
                          "[IDJualD]=" & NullToLong(txtPenjualan.EditValue) & ", " & vbCrLf & _
