...
Public Class frmLaporanPenjualanPalingLaku
    ...
    Private Sub RefreshData()
        Dim ds As New DataSet
        Try
            SQL = "SELECT HBarang.NoID, HBarang.Kode, HBarang.Barcode, HBarang.Nama, HKategori.Nama AS Kategori, SUM(HJualD.Qty*HJualD.Konversi) AS TotalQty, SUM(HJualD.Jumlah) AS Total" & vbCrLf & _
                  " FROM HJual " & vbCrLf & _
                  ...
                  " GROUP BY HBarang.NoID, HBarang.Kode, HBarang.Barcode, HBarang.Nama, HKategori.Nama " & vbCrLf & _
                  " ORDER BY " & IIf(rbQty.Checked, " SUM(HJualD.Qty*HJualD.Konversi) DESC", "SUM(HJualD.Jumlah) DESC")
            EksekusiDataset(ds, "Data", SQL)
            ...
            CC1.Series(0).ArgumentDataMember = "Nama"
            ...
            CC1.Series(0).TopNOptions.Enabled = True
            CC1.Series(0).TopNOptions.Count = NullTolInt(txtValue.EditValue)
            CC1.Legend.Visible = False
            If txtIDKategori.Text <> "" Then
                CC1.Titles(0).Text = txtValue.EditValue & " Penjualan Paling Laku [" & txtIDKategori.Text & "]"
            Else
                CC1.Titles(0).Text = txtValue.EditValue & " Penjualan Paling Laku "
            End If
            CC1.RefreshData()
    ...
    Private Sub cmdPreview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPreview.Click
        Dim NamaFile As String = Application.StartupPath & "\Report\LaporanPenjualanPalingLaku.repx"
        Try
            ViewXtraReport(Me.MdiParent, IIf(IsEditReport, Action_.Edit, Action_.Preview), NamaFile, "Laporan Penjualan Paling Laku", "LaporanPenjualanPalingLaku.repx", dsReport, "A4", "TglDari=Datetime=#" & DateEdit1.DateTime.ToString("yyyy-MM-dd") & "#|TglSampai=Datetime=#" & DateEdit2.DateTime.ToString("yyyy-MM-dd") & "#|KategoriBarang=String='" & FixApostropi(txtIDKategori.Text) & "'|Berdasarkan=String='" & FixApostropi(IIf(rbQty.Checked, "Urut Berdasarkan Qty Penjualan", "Urut Berdasarkan Total Omset Penjualan")) & "'")
        Catch ex As Exception
            XtraMessageBox.Show("Info Kesalahan : " & ex.Message, NamaAplikasi, MessageBoxButtons.OK, MessageBoxIcon.Information)
        End Try
    End Sub

    Private Sub txtValue_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtValue.EditValueChanged
        If Not IsLoad Then
            RefreshData()
        End If
    End Sub
