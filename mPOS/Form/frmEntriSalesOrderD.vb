...
Public Class frmEntriSalesOrderD
    Public NoID As Long
    ...
    Private Sub frmEntriUser_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            Else
                SQL = "SELECT * FROM HSalesOrderD WHERE NoID= " & NoID
                EksekusiDataset(ds, "HSalesOrderD", SQL)
                If ds.Tables("HSalesOrderD").Rows.Count >= 1 Then
                    txtIDBarang.EditValue = NullToLong(ds.Tables("HSalesOrderD").Rows(0).Item("IDBarang"))
                    txtSatuan.EditValue = NullToLong(ds.Tables("HSalesOrderD").Rows(0).Item("IDSatuan"))
                    txtKonversi.EditValue = NullToDbl(ds.Tables("HSalesOrderD").Rows(0).Item("Konversi"))
                    txtQty.EditValue = NullToDbl(ds.Tables("HSalesOrderD").Rows(0).Item("Qty"))
                    txtHarga.EditValue = NullToDbl(ds.Tables("HSalesOrderD").Rows(0).Item("Harga"))
                    txtDiscProsen1.EditValue = NullToDbl(ds.Tables("HSalesOrderD").Rows(0).Item("DiscProsen1"))
                    txtDiscProsen2.EditValue = NullToDbl(ds.Tables("HSalesOrderD").Rows(0).Item("DiscProsen2"))
                    txtDiscRp.EditValue = NullToDbl(ds.Tables("HSalesOrderD").Rows(0).Item("DiscRp"))
                    txtJumlah.EditValue = NullToDbl(ds.Tables("HSalesOrderD").Rows(0).Item("Jumlah"))
                    txtKeterangan.Text = NullToStr(ds.Tables("HSalesOrderD").Rows(0).Item("Keterangan"))
                End If
            End If
            HitungTotal()
    ...
    Private Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
        Try
            If IsValidasi() Then
                If pStatusBaru Then
                    NoID = GetNewID("HSalesOrderD")
                    SQL = "INSERT INTO [HSalesOrderD] ([NoID],[IDHeader],[IDBarang],[IDSatuan],[Konversi],[Qty],[Harga],[DiscProsen1],[DiscProsen2],[DiscRp],[Jumlah],[Keterangan]) VALUES (" & vbCrLf & _
                ...
                Else
                    SQL = "UPDATE [HSalesOrderD] SET " & vbCrLf & _
