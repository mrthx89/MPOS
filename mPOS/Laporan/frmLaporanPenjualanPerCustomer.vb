...
Public Class frmLaporanPenjualanPerCustomer
    ...
    Private Sub RefreshData()
        Dim ds As New DataSet
        Try
            SQL = "SELECT HBarang.NoID, HKontak.Kode + ' ' + HKontak.Nama AS Customer, HBarang.Kode, HBarang.Barcode, HBarang.Nama, HKategori.Nama AS Kategori, SUM(HJualD.Qty*HJualD.Konversi) AS QtyPcs, SUM(HJualD.Jumlah) AS Total" & vbCrLf & _
                  " FROM HJual " & vbCrLf & _
            ...
            SQL = "SELECT HKontak.Kode + ' ' + HKontak.Nama AS Customer, SUM(HJualD.Jumlah) AS Total" & vbCrLf & _
                  " FROM HJual " & vbCrLf & _
                  " INNER JOIN HJualD ON HJual.NoID=HJualD.IDHeader " & vbCrLf & _
                  " INNER JOIN (HBarang LEFT JOIN HKategori ON HKategori.NoID=HBarang.IDKategori) ON HBarang.NoID=HJualD.IDBarang " & vbCrLf & _
                  " INNER JOIN HKontak ON HKontak.NoID=HJual.IDCustomer " & vbCrLf & _
                  ...
                  " ORDER BY SUM(HJualD.Jumlah) DESC"
            EksekusiDataset(ds, "Customer", SQL)
            CC1.Series(0).DataSource = ds.Tables("Customer")
            CC1.Series(0).ArgumentDataMember = "Customer"
            ...
            If txtIDKategori.Text <> "" Then
                CC1.Titles(0).Text = "Penjualan Per Customer [" & txtIDKategori.Text & "]"
            Else
                CC1.Titles(0).Text = "Penjualan Per Customer"
            End If
            CC1.RefreshData()
    ...
    Private Sub cmdPreview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPreview.Click
        Dim NamaFile As String = Application.StartupPath & "\Report\LaporanPenjualanPerCustomer.repx"
        Try
            ViewXtraReport(Me.MdiParent, IIf(IsEditReport, Action_.Edit, Action_.Preview), NamaFile, "Laporan Penjualan Per Customer", "LaporanPenjualanPerCustomer.repx", dsReport, "A4", "TglDari=Datetime=#" & DateEdit1.DateTime.ToString("yyyy-MM-dd") & "#|TglSampai=Datetime=#" & DateEdit2.DateTime.ToString("yyyy-MM-dd") & "#|KategoriBarang=String='" & FixApostropi(txtIDKategori.Text) & "'")
        Catch ex As Exception
            XtraMessageBox.Show("Info Kesalahan : " & ex.Message, NamaAplikasi, MessageBoxButtons.OK, MessageBoxIcon.Information)
        End Try
    End Sub
